...
class walletTransfer_preset_check {
    ...
    async exec() {
        try {
            ...
            // 创建逐行读取的接口
            const rl = fs.createReadStream('batchHgetAllValue_account_repeat.txt');
            // 設置編碼爲 utf8。
            rl.setEncoding("UTF8");
            ...
            rl.on('line', (line)=>{ 
                ...
                if (this.allAccounts[obj.account]) { //若已存在代表完全一樣的account
                    ...
                    obj.rid_preSet_noInsert.push(lineArray[2]['value'])
                ...
                obj.rid_preSet_noInsert  = [lineArray[2]['value']]
            ...
            rl.on('end', async () => {   
                ...
                accountsInserted.forEach(e => {
                    ...
                    if (this.allAccounts[obj.account]) { //若已存在代表完全一樣的account
                        this.totallySameAccount.add(obj.account);
                        obj.rid_preSet_Insert.push(lineArray[2]['value'])
                        obj.hasTotallySameAccount++
                        return;
                    }
                    ...
                    obj.rid_preSet_Insert = [lineArray[2]['value']]
                    obj.rid_updateDate_noInsert  = []
                    obj.rid_updateDate_Insert = []
                    obj.hasTotallySameAccount = 1 

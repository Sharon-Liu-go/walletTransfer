...
class walletTransfer_update {
    ...
    async bashInsertWallet(players_values, player_info_values) {
        ...
        try {
            ...
            console.log(`redis 已掃描rid數: ${this.redisScaned} , 成功insert wallet 總筆數: ${this.affectedRows}, 資料有誤總筆數:${this.failAndNoNeedreTry}, insertDuplicates:${this.insertDuplicates}`)
            console.log('uniqueAccount set:', this.uniqueAccount.size)

            if (this.finishRedisScan && (this.updatePlayer === (this.affectedRows + this.failAndNoNeedreTry + this.insertDuplicates))) {
            ...
            if (players_values.length <= 1) {
                ...
                if (this.finishRedisScan && (this.redisScaned === (this.affectedRows + this.failAndNoNeedreTry))) {
